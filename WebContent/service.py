                    ...
                    if self.length_buffer - g_header_length < g_code_length:
                        ...
                        if recv_message == "quit":
                            ...
                            if (section == '0'):
                                ...
                                img.save(Portindex + '_a.png')
                                #                            os.system("py run.py config Spotlight --words data/melody_words.txt -v resnet")
                                subprocess.run(
                                    "python3 run.py config Spotlight --words data/melody_words.txt -v resnet",
                                    shell=True)
                                child = subprocess.Popen(
                                    "python3 run.py visbeam -s main.167 -m rnn -f agent.167 -i" + Portindex + "_a.png --agent_hs 64 -W 128 -H 64",
                                ...
                                while child.poll() is None:
                                    ...
                                    if line:
                                        ...
                                        melody = line['seq'][:-8].replace("blank", " ")
                                        print(melody)
                                        roadspot = line["spot"]
                                        for i in range(0, len(roadspot)):
                                            if (roadseq[i] == "blank"):
                                                roadseq[i] = " "
                                            sendMessage(
                                                "image:" + str(roadspot[i][0]) + "," + str(roadspot[i][1]) + "," + str(
                                                    roadspot[i][2]) + "," + roadseq[i], Portindex)
                                            time.sleep(1)
                                        line = line["seq"][:-8]
                                        line = line.replace("<", "&lt;")
                                        line = line.replace(">", "&gt;")
                                ...
                                img.save(Portindex + '_a.png')
                                subprocess.run(
                                    "python3 run.py config Spotlight --words data/formula_words.txt -v resnet",
                                    shell=True)
                                child = subprocess.Popen(
                                    "python3 run.py visbeam -s main.3 -m rnn -f agent.3 -i " + Portindex + "_a.png --agent_hs 64 -W 256 -H 128",
                                    shell=True, stdout=subprocess.PIPE)
                                while child.poll() is None:
                                    line = child.stdout.readline()
                                    line = line.strip().decode("utf-8")
                                ...
                                subprocess.run(
                                    ...
                                    shell=True)
                                child = subprocess.Popen(
                                    "python3 run.py visbeam -s main.167 -m rnn -f agent.167 -i example.png --agent_hs 64 -W 128 -H 64",
                                    shell=True, stdout=subprocess.PIPE)
                                while child.poll() is None:
                                    line = child.stdout.readline()
                                    line = line.strip().decode("utf-8")
                        ...
                        if (section == '0' or section == '3'):
                            f = open(Portindex + "_a.txt", 'w', encoding='utf-8')
                            print(
                                "\score {\\version \"2.14.2\"{" + melody + "}\midi {\context {\ScoretempoWholesPerMinute = #(ly:make-moment 72 4)}}}",
                                file=f)
                            f.close()
                            if os.path.exists(Portindex + '_a.ly'):
                                os.remove(Portindex + '_a.ly')
                            os.rename(Portindex + '_a.txt', Portindex + '_a.ly')
                            subprocess.run("lilypond " + str(Portindex) + "_a.ly", shell=True)
